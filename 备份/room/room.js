...
Page({
  data: {
    ...
    voteresult: [],
    roundnumber: 0,
    toView: 'that.acquiredclue[0].content',
    // tab切换    
    currentTab: 0,
    hostname: '',
    usernickname: '',
  ...
  tapnext: function (e) {
    ...
    for (var i = 0; i < that.data.acquiredclue.length; ++i) {
      if (that.data.acquiredclue[i].content === this.data.toView) {
        this.setData({
          toView: this.data.acquiredclue[i + 1].content
        })
        break
      }
    }
  ...
  taplast: function (e) {
    ...
    for (var i = 0; i < that.data.acquiredclue.length; ++i) {
      if (that.data.acquiredclue[i].content === this.data.toView) {
        this.setData({
          toView: that.data.acquiredclue[i - 1].content
        })
        break
      }
    }
